...
async function createTemplate() {
  ...
  const ws = wb.addWorksheet("线路信息采集");
  ...
  titleCell.value = "径迹 — 徒步线路信息采集模板";
  titleCell.font = { name: "Microsoft YaHei", bold: true, size: 16, color: { argb: "FF2E7D32" } };
  titleCell.alignment = { horizontal: "center", vertical: "middle" };
  ws.getRow(1).height = 40;

  // Headers
  const headers = ["分类", "字段名", "填写说明", "示例", "对装备推荐的影响"];
  ...
  const data = [
    ...
    ["基本信息", "线路简介", "100-200字的整体介绍", "世界公认的神山...", "页面展示"],
    ...
    ["地形与路况", "悬崖/暴露感", "是否有悬崖边缘、暴露感强的路段", "无明显悬崖路段", "决定恐高者是否需要心理准备"],
    ...
    ["气候与天气", "降水特点", "雨季时间/降水频率/突发天气", "午后常有阵雨，高海拔天气多变", "决定雨具需求"],
  ...
  data.forEach(([category, field, desc, example, impact]) => {
    ...
    const fill = row % 2 === 0 ? altFill : null;
    [category, field, desc, example, impact].forEach((val, i) => {
  ...
  noteCell.value =
    "提示：每条线路请单独复制一个 sheet 填写。标注「多选」的字段请用顿号分隔。对装备推荐有直接影响的字段请优先填写。";
  noteCell.font = noteFont;
  noteCell.alignment = { wrapText: true };

  // ========== Sheet 2: 示例 ==========
  const ws2 = wb.addWorksheet("示例 — 冈仁波齐转山");
  ws2.columns = [{ width: 18 }, { width: 20 }, { width: 55 }];

  ws2.mergeCells("A1:C1");
  ws2.getCell("A1").value = "示例：冈仁波齐转山";
  ws2.getCell("A1").font = { name: "Microsoft YaHei", bold: true, size: 14, color: { argb: "FF2E7D32" } };
  ws2.getCell("A1").alignment = { horizontal: "center" };
  ws2.getRow(1).height = 35;

  ["分类", "字段名", "填写内容"].forEach((h, i) => {
  ...
  let r = 3;
  let curCat = null;
  exampleData.forEach(([cat, field, value]) => {
    if (cat !== curCat) {
      ...
      for (let col = 1; col <= 3; col++) {
        ws2.getCell(r, col).border = thinBorder;
        ws2.getCell(r, col).fill = categoryFill;
      }
      r++;
      curCat = cat;
    }
    const fill = r % 2 === 0 ? altFill : null;
    [cat, field, value].forEach((v, i) => {
  ...
  await wb.xlsx.writeFile("docs/trail-info-template.xlsx");
  console.log("Excel template created: docs/trail-info-template.xlsx");
}
